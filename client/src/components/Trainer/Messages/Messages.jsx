...
const fetchMessages = (userId, clientId) => {
    if (clientId) {
        return axios.get(`/api/messages/getMessages/${userId}?clientId=${clientId}`);
    } else {
        return axios.get(`/api/messages/getMessages/${userId}`);
    }
};

const fetchClientsForTrainer = (trainerId) => axios.get(`/api/user/listAllClientsForTrainer/${trainerId}`);
...
const ClientList = ({ clients, currentClientId, onClientSelect }) => (
    <div className="border-r border-gray-300 flex-initial w-1/4 space-y-4 pr-4 overflow-y-auto">
        ...
        {clients.map(client => (
            <div
                key={client.uid}
                className={`mb-3 p-4 flex items-center cursor-pointer hover:bg-gray-200 ${currentClientId === client.uid ? 'bg-gray-100' : ''}`}
                onClick={() => onClientSelect(client.uid)}
            >
                <img src={client.profilePic || 'https://via.placeholder.com/40'} alt="Client" className="w-10 h-10 rounded-full mr-3" />
...
const Messages = () => {
    ...
    useEffect(() => {
        setError(null);
        Promise.all([fetchMessages(currentUserId), fetchClientsForTrainer(currentUserId)])
            .then(([messagesResponse, clientsResponse]) => {
                ...
                setRecipientId(sortedClients[0]?.uid || "");
                setIsLoading(false);
            })
            .catch(err => {
    ...
    const currentClient = clients.find(c => c.uid === recipientId);
    ...
    return (
        <div className="h-screen w-full bg-background flex items-center justify-center">
            <div className="p-6 bg-white shadow rounded-lg flex h-5/6 w-3/4">
                <ClientList clients={clients} currentClientId={recipientId} onClientSelect={setRecipientId} />
                <div className="flex-grow w-3/4 max-w-3/4 pl-4">
                    {currentClient && (
                        <div className="flex items-center border-b border-gray-300 pb-2 mb-2">
                            <img src={currentClient.profilePic || 'https://via.placeholder.com/40'} alt="Client" className="w-10 h-10 rounded-full mr-3" />
                            <span className="text-lg font-semibold">{currentClient.firstName} {currentClient.lastName}</span>
                        </div>
                    )}
                    <MessageList
                        messages={messages}
